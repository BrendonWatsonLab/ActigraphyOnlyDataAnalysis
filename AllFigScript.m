...
current_metric_var = 'SelectedPixelDifference';
if strcmp(current_metric_var, 'SelectedPixelDifference')
    ...
    if strcmp(current_metric_var, 'SelectedPixelDifference')
        ...
        try
            ...
            for i = 1:length(u_ans)
                ...
                % --- FIX: Robust String Matching for Tau ---
                tau_animals = string({tau_results.Animal});
                tau_idx = (tau_animals == string(an_id));
                
                if ~any(tau_idx)
                    fprintf('   Warning: Could not find Tau match for Animal %s. Skipping...\n', string(an_id));
                    continue; 
                end
                an_tau = tau_results(tau_idx).Tau;
                
                % Calculate CT for last week
                % 1. Get all FullDark data for this animal to find the true start time
                an_fd_data = fd_data(fd_data.Animal == an_id, :);
                start_time = min(an_fd_data.RelativeDay);
                
                % 2. Find the exact ZT Hour when the FullDark condition started
                start_zt_array = an_fd_data.(ztHourVar)(an_fd_data.RelativeDay == start_time);
                start_zt = start_zt_array(1); % Extract the first value
                
                % 3. Calculate elapsed hours since the start of FullDark
                t_hours = (d_last_week.RelativeDay - start_time) * 24;
                
                % 4. Add the starting ZT phase to the elapsed time
                ct_float = mod(t_hours + start_zt, an_tau) * (24 / an_tau);
                d_last_week.CT_Hour = floor(ct_float);
