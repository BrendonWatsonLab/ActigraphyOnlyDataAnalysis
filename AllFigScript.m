...
%% SYMMETRIC OUTLIER DETECTION (For Figure 5) ---
% Detects and removes hyperactive/hypoactive outliers from both sexes in 300Lux
disp('--- Checking for Outliers in Both Sexes (300Lux Data) ---');
try
    sexes_to_check = {'Male', 'Female'};
    sex_lists = {maleAnimals, femaleAnimals};
    
    for s = 1:length(sexes_to_check)
        current_sex = sexes_to_check{s};
        current_list = sex_lists{s};
        
        % 1. Calculate Total Daily Activity for this sex in 300 Lux
        sex_300_data = dataTable(ismember(dataTable.Animal, current_list) & dataTable.Condition == '300Lux', :);
        if isempty(sex_300_data), continue; end
        
        % Mean of bins approximates total activity level
        daily_sums = groupsummary(sex_300_data, 'Animal', 'mean', current_metric_var); 
        
        % 2. Detect Outlier (Using 'quartiles' is robust for small N)
        [tf, ~] = isoutlier(daily_sums.(['mean_', current_metric_var]), 'quartiles');
        
        if any(tf)
            outlier_ids = string(daily_sums.Animal(tf));
            for o_idx = 1:length(outlier_ids)
                fprintf('*** DETECTED %s OUTLIER: %s ***\n', upper(current_sex), outlier_ids(o_idx));
                fprintf('Removing %s from %s analysis lists.\n', outlier_ids(o_idx), current_sex);
                
                % Remove from the specific sex list
                if strcmp(current_sex, 'Male')
                    maleAnimals = maleAnimals(maleAnimals ~= outlier_ids(o_idx));
                elseif strcmp(current_sex, 'Female')
                    femaleAnimals = femaleAnimals(femaleAnimals ~= outlier_ids(o_idx));
                end
                
                % Could remove from the overall dataset too
                % (Optional: fourCondAnimals = fourCondAnimals(fourCondAnimals ~= outlier_ids(o_idx)); )
            end
        else
            fprintf('No statistical outliers detected in %s 300Lux group.\n', current_sex);
        end
    end
    
    % Re-define sex groups with the clean lists
    sex_groups = {maleAnimals, femaleAnimals};
    
catch ME_Outlier
    warning('ErrGen:Outlier', 'Could not run outlier detection: %s', ME_Outlier.message); 
end
...
for met_idx = 1:length(activity_metrics_to_plot)
    ...
    %% FIG 5A: Sex differences in 24h profiles (WITH HOURLY STATS)
    ...
    for sex_idx = 1:length(sex_groups)
        ...
        % Store profiles for 300 and 1000 to place asterisks
        prof_300 = []; prof_1000 = [];
        ...
        for i_cond = 1:length(conditionsFor5A)
            ...
            % Save for plotting asterisks later
        ...
        % Y-Limits
        if strcmp(current_metric_var, 'NormalizedActivity'), ylim(ax5A, [-0.4, 1.2]); end
        if strcmp(current_metric_var, 'SelectedPixelDifference'), ylim(ax5A, [0, 8E5]); end
        ...
        % --- HOURLY STATS (300 vs 1000) ---
        fprintf('-- Stats for Figure 5A (%s) --\n', sex_labels{sex_idx});
        p_values = ones(24, 1);
        for zt = 0:23
            ...
            % Use Paired ttest if N is consistent, else ttest2
            if ~isempty(d300) && ~isempty(d1000)
                if length(d300) == length(d1000), [~, p] = ttest(d300, d1000); else, [~, p] = ttest2(d300, d1000); end
                p_values(zt+1) = p; 
            end
        end
        
        alpha_5A = 0.05 / 24; % Bonferroni
        ...
        if ~isempty(sig_idx)
            ...
            fprintf('  Significant differences (p < %.4f): ZT %s\n', alpha_5A, num2str(zt_sig'));
        end
...
%% FIG 5B: Diurnality Violins by Sex and Condition (12-Violin Plot) WITH DESCRIPTIVE STATS
    ...
    %% FIG 5C: Difference Bar Plot (Male Change vs Female Change)
    ...
    try
        ...
        % Means for bar plot
        m_mean = mean(male_diffs, 1, 'omitnan');
        ...
        if ~isempty(sig_idx)
            ...
            plot(ax5C, zt_sig, y_tops + 0.05*y_range, '*', 'Color', 'k', 'MarkerSize', 6);
            fprintf('Fig 5C Significant interaction (p < %.4f): ZT %s\n', alpha_5C, num2str(zt_sig'));
        end
        
        title(sprintf('Fig 5C: Activity Change (1000L - 300L) by Sex (%s)', current_metric_suffix));
    ...
    %% FIG 5F: Peak Activity Shift in FullDark by Sex (CIRCULAR STATS + CoG)
    disp('--- Starting Figure 5F: Peak Shift by Sex (Center of Gravity) ---');
    try
        ...
        fprintf('\n--- STATS: Figure 5F (Circular Shift T-Test - CoG) ---\n');
        ...
        for i_sex = 1:length(sex_labels)
            ...
            % 1. Calculate Daily Peaks per Animal using CoG
            ...
            % 2. Plot Pooled Line (Visual only) using Circular Mean of the CoGs
            % We need to average the daily peaks across animals CIRCULARLY
            calc_circ_mean_vec = @(x) mod(atan2(mean(sin(x*2*pi/24),1,'omitnan'), mean(cos(x*2*pi/24),1,'omitnan')) * 24/(2*pi), 24);
            
            % Transpose to get rows=animals, so we mean across columns (if calc_circ_mean works on columns)
            % Actually, let's just do it manually row by row (day by day) to be safe
            daily_pool = NaN(length(unique_days_FD), 1);
            for d = 1:length(unique_days_FD)
                daily_vals = animal_daily_peaks(d, :);
                if any(~isnan(daily_vals))
                    daily_pool(d) = calc_circ_mean_vec(daily_vals'); % Vector input
                end
            end
            
            plot(ax5F, unique_days_FD, daily_pool, 'o-', 'Color', sex_color, 'LineWidth', 1.5, 'DisplayName', sex_str);
            
            % 3. Run Stats
            fprintf('--- Stats for %s ---\n', sex_str);
            min_day_idx = 1; max_day_idx = length(unique_days_FD);
            if max_day_idx >= 7 && N_sex >= 3 % Lowered N requirement slightly to allow for small groups
                first_week_indices = 1:7; 
                last_week_indices = (max_day_idx-6):max_day_idx;
                ...
                % Circular Means per Animal (averaging their CoGs over the week)
        ...
        fig_filename = fullfile(savePath_Fig5, sprintf('Figure5F_PeakShift_bySex_CoG_%s.png', current_metric_suffix));
    ...
    try
        hFig5G_current = figure('Name', sprintf('Fig 5G: Phase Pct Change (%s)', current_metric_suffix), 'Visible', 'off', 'Color', 'w');
        ax5G = axes('Parent', hFig5G_current); hold(ax5G, 'on');
        
        % Comparison Pairs definition: {Name, BaselineCond, TargetCond, Phase}
        comparisons = {
            '1000L-ON', '300Lux', '1000Lux', 'On';
            'Dark-ON',  '300Lux', 'FullDark', 'On';
            '1000L-OFF', '300Lux', '1000Lux', 'Off';
            'Dark-OFF', '300Lux', 'FullDark', 'Off';
        };
        ...
        for i_comp = 1:size(comparisons, 1)
            ...
            % (Intersect with sex groups later)
            
            for i_an = 1:length(animals_to_use)
                an_id = animals_to_use(i_an);
                % Get Baseline
                d_base = dataTable(dataTable.Animal==an_id & dataTable.Condition==base_cond & dataTable.LightPeriod==phase, :).(current_metric_var);
                val_base = mean(d_base, 'omitnan');
                % Get Target
                d_targ = dataTable(dataTable.Animal==an_id & dataTable.Condition==targ_cond & dataTable.LightPeriod==phase, :).(current_metric_var);
                val_targ = mean(d_targ, 'omitnan');
                ...
                if ~isnan(val_base) && val_base ~= 0 && ~isnan(val_targ)
                    ...
                    % Store comparison label as STRING to avoid cell array errors
        ...
        % Plot Violins
        % Create composite group for violinplot (Comp-Sex)
        plotTable_5G.Group = categorical(strcat(plotTable_5G.Comparison, '-', plotTable_5G.Sex));
        
        % Define Order: 1000L-ON-F, 1000L-ON-M, Dark-ON-F, Dark-ON-M...
        grp_order = {};
        ...
        disp('--- STATS: Figure 5G (Sex Differences in Phase Response) ---');
        y_lims = ylim(ax5G); y_range = diff(y_lims);
        y_pos = y_lims(2) + 0.05*y_range;
        
        for i_c = 1:size(comparisons, 1)
            comp_name = string(comparisons{i_c, 1}); % Ensure string for comparison
            
            % Robust filtering using strings
            ...
            if ~isempty(data_m) && ~isempty(data_f)
                ...
                % Plot bar if significant (Alpha 0.05 for planned comparison)
                if p < 0.05
        ...
        title(ax5G, sprintf('Phase-Specific Response by Sex (%s)', strrep(current_metric_suffix,'_',' ')));
        xtickangle(ax5G, 45);
        set(ax5G, 'Position', [0.1 0.25 0.88 0.65]);
        
        fig_filename = fullfile(savePath_Fig5, sprintf('Figure5G_PhaseResponse_%s.png', current_metric_suffix));
