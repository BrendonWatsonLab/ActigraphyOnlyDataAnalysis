...
for met_idx = 1:length(activity_metrics_to_plot)
    ...
    try
        ...
        p_values = ones(24, 1);
        for zt = 0:23
            % Extract small tables with Animal ID
            T_300 = dataTable(ismember(dataTable.Animal, animals_for_cond) & ...
                              dataTable.Condition == '300Lux' & ...
                              dataTable.ZT_Time == zt, {'Animal', current_metric_var});
            
            T_1000 = dataTable(ismember(dataTable.Animal, animals_for_cond) & ...
                               dataTable.Condition == '1000Lux' & ...
                               dataTable.ZT_Time == zt, {'Animal', current_metric_var});
            
            % Inner Join ensures rows align perfectly by Animal ID
            % Variable names become 'Var_Left' and 'Var_Right' (or similar)
            T_Paired = innerjoin(T_300, T_1000, 'Keys', 'Animal');
            
            % Access columns dynamically (usually cols 2 and 3 after join)
            % Column 2 is 300Lux, Column 3 is 1000Lux
            if height(T_Paired) >= 3
                d1 = T_Paired{:, 2}; 
                d2 = T_Paired{:, 3};
                [~, p] = ttest(d1, d2); % Paired T-Test
                p_values(zt+1) = p;
            end
        end
        
        % 5. PLOT ASTERISKS
        alpha_4B = 0.05; % No Bonferroni Correction
...
for met_idx = 1:length(activity_metrics_to_plot)
    ...
    %% FIG 5A: Sex differences in 24h profiles (INDIVIDUAL TRACES + SD SHADING)
    ...
    for sex_idx = 1:length(sex_groups)
        ...
        prof_300 = []; prof_1000 = []; % For asterisk logic later
        ...
        for i_cond = 1:length(conditionsFor5A)
            ...
            % 1. Plot Individual Traces (Light and Thin)
            ...
            fill(ax5A, [(0:23)'; flipud((0:23)')], [meanProfile-sdProfile; flipud(meanProfile+sdProfile)], ...
                condColors_5A{i_cond}, 'FaceAlpha', 0.1, 'EdgeColor', 'none', 'HandleVisibility', 'off');
        end
        
        % Formatting
        if strcmp(current_metric_var, 'NormalizedActivity'), ylim(ax5A, [-1.5, 3]); end % Expanded Y-lim for individual traces
        current_ylim = ylim(ax5A);
        
        % Shading for Dark Phase
